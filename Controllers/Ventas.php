	...
	public function getVentas()
	{
		if ($_SESSION['permisosMod']['r']) {
			...
			for ($i = 0; $i < count($arrData); $i++) {
				...
				if ($_SESSION['permisosMod']['r']) {
					$btnView = '<button class="btn btn-info btn-sm btnViewVenta" onClick="fntViewVenta(' . $arrData[$i]['cod_factura'] . ')" title="Ver Venta"><i class="far fa-eye"></i></button>';
				}
				...
				if ($_SESSION['permisosMod']['d']) {
					$btnDelete = '<button class="btn btn-danger btn-sm btnDelVenta" onClick="fntDelVenta(' . $arrData[$i]['cod_factura'] . ')" title="Eliminar Venta"><i class="far fa-trash-alt"></i></button>';
				} else {
