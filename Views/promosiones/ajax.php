...
if (!empty($_POST)) {
	...
	if ($_POST['action'] == 'addCliente') {
		$nombre_promocion       = $_POST['txtnombre_promocion'];
		$fecha_inicio    = $_POST['txtfecha_inicio'];
		$fecha_final  = $_POST['txtfecha_final'];
		$precio_venta = $_POST['txtprecio_venta'];

		$query_insert = mysqli_query($conection, "INSERT INTO tbl_promocion(
														nombre_promocion,fecha_inicio,fecha_final,precio_venta)
													VALUES('$nombre_promocion','$fecha_inicio','$fecha_final','$precio_venta')");
		if ($query_insert) {
		...
		if ($result > 0) {
			$query_procesar = mysqli_query($conection, "call procesar_promocion($usuario,'$token',$codproducto', $cantidadproducto,$nombre_promocion','$fecha_i','$fecha_f' '$precio_venta')");
	...
	if ($_POST['action'] == 'addProductoDetalle') {
		if (empty($_POST['producto']) || empty($_POST['cantidad'])) {
			...
			$query_detalle_temp 	= mysqli_query($conection, "CALL add_detalle_temp_p($cod_producto,$cantidad,'$token')");
	...
	if ($_POST['action'] == 'delProductoDetalle') {
		if (empty($_POST['id_detalle'])) {
			...
			$query_detalle_temp 	= mysqli_query($conection, "CALL del_detalle_temp_p($id_detalle,'$token')");
			$result = mysqli_num_rows($query_detalle_temp);
			...
			if ($result > 0) {
				...
				while ($data = mysqli_fetch_assoc($query_detalle_temp)) {
					...
					$detalleTabla .= '<tr>
											...
											<td class="">
												<a class="link_delete" href="#" onclick="event.preventDefault(); del_product_detalle_p(' . $data['cod_detalle_produccion'] . ');"><i class="far fa-trash-alt"></i></a>
											</td>
	...
	if ($_POST['action'] == 'anularVenta') {
		...
		$query_del = mysqli_query($conection, "DELETE FROM detalle_temp_p WHERE token_user = '$token' ");
		mysqli_close($conection);
		if ($query_del) {
			echo 'ok';
		} else {
	...
	if ($_POST['action'] == 'procesarVenta') {
		...
		if ($result > 0) {
			$query_procesar = mysqli_query($conection, "CALL procesar_promo($usuario,'$token')");
			$result_detalle = mysqli_num_rows($query_procesar);
