...
if (empty($_REQUEST['id'])) {
	...
	$query_producto = mysqli_query($conection, "SELECT p.cod_producto,p.descripcion,p.precio,p.foto,pr.codproveedor,pr.proveedor
														FROM tbl_producto p
														INNER JOIN proveedor pr
														ON p.proveedor = pr.codproveedor
														WHERE p.cod_producto = $id_producto AND p.estado = 1");
